...
impl TestHost {
    ...
    /// Do the `--gate` hook's request/response over the real socket and return once the host has
    /// applied it. Drives the same client the real hook uses (`hook::query_gate`) so the harness
    /// exercises it, with a generous timeout so a loaded machine does not flake the verdict.
    pub fn gate_query(&self, session: &str) -> Verdict {
        let before = self.draws.load(Ordering::Acquire);
        let verdict =
            crate::hook::query_gate(&self.socket_path, session, StdDuration::from_secs(5));
