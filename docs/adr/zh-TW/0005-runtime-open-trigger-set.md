...
**決策。** Runtime 維護目前 open 的 Trigger 集合，以 adapter 和 agent session 作為 key。開啟 Trigger 會新增一個 entry；關閉 Trigger（Stop / PermissionRequest / Elicitation）會移除該 entry。只要集合非空，開發者就正好處於「等待中」：集合非空時浮出卡片，集合變空時清掉卡片。
